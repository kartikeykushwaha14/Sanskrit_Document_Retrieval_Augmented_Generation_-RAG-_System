...
class TFIDFRetriever:
    ...
    def retrieve(self, query: str, top_k: int = 3) -> List[Tuple[int, float, str]]:
        ...
        # Convert doc_vectors to numpy array for batch processing
        if len(self.doc_vectors) > 0:
            # Stack all document vectors into a matrix
            doc_matrix = np.vstack(self.doc_vectors)

            # Compute all similarities at once (vectorized - MUCH faster!)
            similarities = np.dot(doc_matrix, query_vector)

            # Get top-k indices (partial sort - faster than full sort for large datasets)
...
class SanskritRAGSystem:
    ...
    def ingest_documents(self, file_paths: List[str], append: bool = True):
        """
        Ingest and process multiple documents

        Args:
            file_paths: List of document file paths
            append: If True, add to existing chunks. If False, replace all chunks.
        """
        print("\n📚 Ingesting Documents...")
        print("-" * 60)

        # Start chunk_id from current max if appending
        if append and self.chunks:
            chunk_id = max(chunk.chunk_id for chunk in self.chunks) + 1
            print(f"Appending to existing {len(self.chunks)} chunks")
        else:
            chunk_id = 0
            if not append:
        ...
        print(f"\n✓ New chunks created: {len(new_chunks)}")
        print(f"✓ Total chunks in system: {len(self.chunks)}")
        print(f"✓ Total documents: {len(file_paths)} new, {len(self.raw_documents)} total")
